...
double[,] GetRandomMatrix(int rows, int columns, int leftRange, int rightRange)
{
    ...
    for(int i = 0; i < matrix.GetLength(0); i++)
    {
        ...
        {
          matrix[i, j] = new Random().NextDouble()*(rightRange - leftRange) + leftRange;
        }
    }
...
void PrintMatrix(double[,] matrix)
{
    for(int i = 0; i < matrix.GetLength(0); i++)
    {
        for(int j = 0; j < matrix.GetLength(1); j++)
        {
            Console.Write(String.Format("{0,5:f1}",matrix[i,j])); // нашел этот способ, так столбцы выводятся более прямыми
